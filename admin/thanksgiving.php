<?php include"header.php"; ?>
	<!-- //header-ends -->
			<div id="page-wrapper">
				<div class="graphs">
					<h3 class="blank1">All Thanksgiving Submissions</h3>
					 <div class="xs tabls">
					    ...
					    <thead>
                          ...
                          //include('../connect.php');
                            if(!isset($_GET["page"])){
                                ...
                                $tbl_name="thanksgiving";		//your table name
	...
	*/
        $get_thanksgiving_tbl = ORM::for_table("$tbl_name")
                 ->find_array();
         
	$total_pages = count($get_thanksgiving_tbl);
	...
	/* Get data. */
				...
				$result = $db->prepare("SELECT * FROM thanksgiving ORDER BY id DESC LIMIT $start, $limit");
				$result->execute();
		...
		for($i=1; $row = $result->fetch(); $i++){
               ...
               ?> 
						  ...
						  <tr>
                            ...
                            <td><?php echo $row['thanksgiving']; ?></td>
                            <td><?php echo $row['anonymous']  ? 'Yes' : 'No'; ?></td>
							 ...
							 <?php } ?>
                             <a class="btn btn-default">Download<i class="fa fa-download"></i></a>
						</table>
						</div>
						</div>
				</div>
